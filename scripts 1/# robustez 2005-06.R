...
df_aborto <- subset(df, select=c(p65,partido))
...
df_aborto <- complete6(p65,partido)
summary(df_aborto)
...
df_aborto$contraaborto <- scales::rescale(df_aborto$p65, to = c(0, 1))
...
merge$Período <- "2005-2006"
summary(merge)
table(merge$Dimensão)
write.csv(merge,
          "C:/Users/grego/OneDrive/Desktop/work/Congruencia so brasil/graficos oscilacao no tempo/mer05_06.csv")
